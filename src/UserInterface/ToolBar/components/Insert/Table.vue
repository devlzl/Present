...
<template>
  <ButtonGroup :name="$t('ToolBar.insert.table.title')">
    <button class="relative flex flex-col items-center py-0 menu-btn" @click.stop="showCreateTableBox = !showCreateTableBox">
      ...
      <div v-if="showCreateTableBox" class="absolute w-[210px] h-[200px] border border-gray-500 top-[70px] left-0 bg-white">
        ...
        <div class="ml-[3px]" @mouseleave="mouseLeaveHandler">
          <div v-for="rowIndex of 8" :key="rowIndex" class="flex justify-center">
            <div
              v-for="colIndex of 10" :key="colIndex + (rowIndex - 1) * 10"
